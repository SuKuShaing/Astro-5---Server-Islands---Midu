...
}, "D:/Progra/Youtube/Astro/Astro 5 - Server Islands - Midu/src/components/AmazonLogo.astro", void 0);
...
const $$BuyButton = createComponent(($$result, $$props, $$slots) => {
  ...
  const storeCountry = country === SPAIN ? "spain" : "chile";
  const countryName = country === SPAIN ? "Espa\xF1a" : "Chile";
  const { buy } = Astro2.props;
  const url = buy[storeCountry];
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(url, "href")} target="_blank" rel="noopener noreferrer" class="text-balance inline-flex gap-2 items-center bg-yellow-400 hover:bg-yellow-500 active:bg-yellow-600 text-black font-bold py-2 px-4 rounded-lg border border-yellow-500 transition duration-200 ease-in-out hover:scale-105 justify-center text-center">
Comprar en Amazon ${countryName} ${renderComponent($$result, "AmazonLogo", $$AmazonLogo, {})} </a>`;
}, "D:/Progra/Youtube/Astro/Astro 5 - Server Islands - Midu/src/components/BuyButton.astro", void 0);

const $$file = "D:/Progra/Youtube/Astro/Astro 5 - Server Islands - Midu/src/components/BuyButton.astro";
const $$url = undefined;
